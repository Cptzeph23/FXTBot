...
from fxtbot.loggers.buffering_handler import FTBufferingHandler
from fxtbot.loggers.ft_rich_handler import FtRichHandler
from fxtbot.loggers.rich_console import get_rich_console


# from fxtbot.loggers.std_err_stream_handler import FTStdErrStreamHandler
...
FT_LOGGING_CONFIG = {
    ...
    "handlers": {
        "console": {
            "class": "fxtbot.loggers.ft_rich_handler.FtRichHandler",
            "formatter": "basic",
        },
    },
    "root": {
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "fxtbot": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "fxtbot.loggers.ft_rich_handler.FtRichHandler":
            ...
            try:
                ...
                raise OperationalException(
                    ...
                    "directories. If you're running fxtbot using docker, you see this error "
