...
from fxtbot.configuration import TimeRange, validate_config_consistency
from fxtbot.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from fxtbot.data import history
from fxtbot.data.btanalysis import find_existing_backtest_stats, trade_list_to_dataframe
from fxtbot.data.converter import trim_dataframe, trim_dataframes
from fxtbot.data.dataprovider import DataProvider
from fxtbot.data.metrics import combined_dataframes_with_rel_mean
...
from fxtbot.ft_types import (
    BacktestContentType,
    BacktestContentTypeIcomplete,
    BacktestResultType,
    get_BacktestResultType_default,
)
from fxtbot.leverage.liquidation_price import update_liquidation_prices
from fxtbot.mixins import LoggingMixin
from fxtbot.optimize.backtest_caching import get_strategy_run_id
from fxtbot.optimize.bt_progress import BTProgress
...
from fxtbot.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from fxtbot.resolvers import ExchangeResolver, StrategyResolver
from fxtbot.strategy.interface import IStrategy
from fxtbot.strategy.strategy_wrapper import strategy_safe_wrapper
from fxtbot.util import FtPrecise, dt_now
from fxtbot.util.migrations import migrate_data
from fxtbot.wallets import Wallets
...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Exchange | None = None) -> None:
        ...
        if self.config.get("freqai_backtest_live_models", False):
            from fxtbot.freqai.utils import get_timerange_backtest_live_models
    ...
    def _get_close_rate_for_roi(
        ...
        if roi is not None and roi_entry is not None:
            ...
            if trade_dur == 0 and (
                ...
                # details: https: // github.com/fxtbot/fxtbot/issues/6261
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # fxtbot does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which fxtbot does not support in live.
