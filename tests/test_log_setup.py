...
from fxtbot.loggers import (
    FTBufferingHandler,
    FtRichHandler,
    setup_logging,
    setup_logging_pre,
)
from fxtbot.loggers.set_log_levels import (
...
def test_set_loggers_json_format(capsys):
    ...
    config = {
        ...
        "log_config": {
            ...
            "formatters": {
                "json": {
                    "()": "fxtbot.loggers.json_formatter.JsonFormatter",
                    "fmt_dict": {
                        "timestamp": "asctime",
                        "level": "levelname",
                        "logger": "name",
...
def test_reduce_verbosity():
    setup_logging_pre()
    reduce_verbosity_for_bias_tester()
    prior_level = logging.getLogger("fxtbot").getEffectiveLevel()

    assert logging.getLogger("fxtbot.resolvers").getEffectiveLevel() == logging.WARNING
    assert logging.getLogger("fxtbot.strategy.hyper").getEffectiveLevel() == logging.WARNING
    # base level wasn't changed
    assert logging.getLogger("fxtbot").getEffectiveLevel() == prior_level

    restore_verbosity_for_bias_tester()

    assert logging.getLogger("fxtbot.resolvers").getEffectiveLevel() == prior_level
    assert logging.getLogger("fxtbot.strategy.hyper").getEffectiveLevel() == prior_level
    assert logging.getLogger("fxtbot").getEffectiveLevel() == prior_level
    # base level wasn't changed
