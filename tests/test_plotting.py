...
from fxtbot.commands import start_plot_dataframe, start_plot_profit
from fxtbot.configuration import TimeRange
from fxtbot.data import history
from fxtbot.data.btanalysis import load_backtest_data
from fxtbot.data.metrics import create_cum_profit
from fxtbot.exceptions import OperationalException
from fxtbot.plot.plotting import (
    add_areas,
    add_indicators,
    add_profit,
    create_plotconfig,
...
def test_generate_candlestick_graph_no_signals_no_trades(default_conf, mocker, testdatadir, caplog):
    row_mock = mocker.patch(
        "fxtbot.plot.plotting.add_indicators", MagicMock(side_effect=fig_generating_mock)
    )
    trades_mock = mocker.patch(
        "fxtbot.plot.plotting.plot_trades", MagicMock(side_effect=fig_generating_mock)
    )
...
def test_generate_Plot_filename():
    ...
    assert fn == "fxtbot-plot-UNITTEST_BTC-5m.html"


def test_generate_plot_file(mocker, caplog, user_dir):
    fig = generate_empty_figure()
    plot_mock = mocker.patch("fxtbot.plot.plotting.plot", MagicMock())
    store_plot_file(
        fig, filename="fxtbot-plot-UNITTEST_BTC-5m.html", directory=user_dir / "plot"
    )

    expected_fn = str(user_dir / "plot/fxtbot-plot-UNITTEST_BTC-5m.html")
...
def test_start_plot_dataframe(mocker):
    aup = mocker.patch("fxtbot.plot.plotting.load_and_plot_trades", MagicMock())
    args = [
        "plot-dataframe",
        "--config",
...
def test_start_plot_profit(mocker):
    aup = mocker.patch("fxtbot.plot.plotting.plot_profit", MagicMock())
    args = [
        "plot-profit",
        "--config",
        "tests/testdata/testconfigs/main_test_config.json",
...
def test_plot_profit(default_conf, mocker, testdatadir):
    ...
    store_mock = MagicMock()
    mocker.patch.multiple(
        "fxtbot.plot.plotting", generate_profit_graph=profit_mock, store_plot_file=store_mock
    )
    with pytest.raises(
